...
DATASET_CONFIGS = {
    "DroneRF": {
        ...
        "model_path": "outputs/resnet_multiclass/models/best_model.pt",
        "holdout_classes": {
            "AR Drone": 1,
            "Bepop Drone": 2,
            "Phantom Drone": 3,
    ...
    "CageDroneRF": {
        "num_classes": 27,
        "class_names": None,  # chargé dynamiquement
        "model_path": "outputs/cagedronerf_resnet_multiclass/models/best_model.pt",
        "holdout_classes": {
            "Skydio_2": None,  # résolu dynamiquement
            "Parrot_Anafi": None,
            "background": None,
            "Hubsan_X4_Air": None,
            "DJI_Tello": None,
    ...
    "RFUAV": {
        "num_classes": 37,
        "class_names": None,  # chargé dynamiquement
        "model_path": "outputs/rfuav_resnet_multiclass/models/best_model.pt",
        "holdout_classes": None,  # sélectionné dynamiquement
    },
}
...
def run_openset_multiclass(device, output_dir):
    ...
    for ds_name, config in DATASET_CONFIGS.items():
        ...
        # Obtenir les noms de classes et le nombre de classes
        if config["class_names"] is not None:
            class_names = config["class_names"]
            num_classes = config["num_classes"]
        else:
        ...
        for cls_name, cls_id in holdout_classes.items():
            try:
                results = run_openset_evaluation(
                    ...
                    output_dir=str(out_base / ds_name / f"holdout_{cls_name}")
                )
...
def main():
    parser = argparse.ArgumentParser(description="Multiclass robustness + open-set evaluation")
